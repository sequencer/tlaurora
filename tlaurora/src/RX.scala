...
/** Take Aurora RX [[UserPDUs]].
  ...
  */
class RX(val parameter: RXParameters) extends Module with SerializableModule[RXParameters] {
  // IO
  // TileLink Master Channels
  val masterBChannel =
    parameter.slaveLinkParameters.channelBParameter.map(p => IO(Flipped(DecoupledIO(new TLChannelB(p)))))
  ...
  val slaveCChannel =
    parameter.masterLinkParameters.channelCParameter.map(p => IO(Flipped(DecoupledIO(new TLChannelC(p)))))
  val slaveEChannel =
    parameter.masterLinkParameters.channelEParameter.map(p => IO(Flipped(DecoupledIO(new TLChannelE(p)))))
}
